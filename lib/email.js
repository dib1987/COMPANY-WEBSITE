...
// ---------------------------------------------------------------------------
export function autoReplyEmail({ name, business, problem }) {
  const fn = firstName(name);
  const biz = business && business.trim() ? business.trim() : "your business";
  const subject = `Got it, ${fn} — your AI automation review is on the way`;

  const text = [
    `Hi ${fn},`,
    ``,
    `Thanks for reaching out. I've got your request for a free AI automation review for ${biz}.`,
    ``,
    problem ? `Here's what you told me you're trying to solve:\n"${problem.trim()}"\n` : ``,
    `What happens next:`,
    `1. I look at how work actually moves through ${biz}.`,
  ...
  const inner = `
    ...
    <p style="margin:0 0 14px;font-size:15px;line-height:1.55;color:#33384a;">Thanks for reaching out. I've got your request for a free AI automation review for <strong>${escapeHtml(biz)}</strong>.</p>
    ...
    <p style="margin:0 0 6px;font-size:15px;line-height:1.55;color:#33384a;">1. I look at how work actually moves through ${escapeHtml(biz)}.</p>
